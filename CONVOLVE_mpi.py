...
def main():
    ...
    # print(f"Hello from rank={rank} out of size={size} on node={host}")
    
    args=parse_arguments()
    if rank==0:
        print('Printing from Rank=0: Starting loading')
        ...
        print('Printing from Rank=0: Loading done')
    else:
    ...
    # chunk_size=(num_samples+size-1)//size  ## Old chunk size, possibly too large
    # chunk_size=(num_samples // (size*2)) ## New chunk size
    chunk_size=1_000_000
    if rank==0: print('Chunk size: ', chunk_size)

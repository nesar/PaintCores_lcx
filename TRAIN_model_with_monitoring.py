...
def preprocess_data(mags, redshift, sfh, wave_min=90, wave_max=50000):
    ...
    # Take log10 of mags, then scale
    mags_log = np.log10(mags)
    mags_scaled = scaler_output.fit_transform(mags_log)
...
def validate_random_points(model, val_data, val_target, scaler_output, n_points=20):
    np.random.seed(10)
    random_indices = np.random.choice(val_data.shape[0], n_points, replace=False)
    for idx in random_indices:
        # Inverse-transform from scaled log10 space, then exponentiate
        true_target_log = scaler_output.inverse_transform(val_target[idx:idx+1])[0]
        model_output_log = scaler_output.inverse_transform(model.predict(val_data[idx:idx+1]))[0]
        true_target = 10**(true_target_log)
        model_output = 10**(model_output_log)
...
def main():
    ...
    hidden_dims = [512, 512, 1204, 512] # [512, 512, 1204, 512, 512]
